        ...
        <div class="TABLE-WITHOUT-SEARCH-BAR p-0">
            ...
            <form id="form_create_pelatihan" class="p-4 pt-3">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" name="kategori" id="pelatihan_id">
                ...
                <div class="form-group" style="margin-top: 15px;">
                    ...
                    <div class="file-upload-container">
                        <input type="file" class="file-input" id="video" name="vidio"
                ...
                <div class="form-group" id="linkFormGroup" style="margin-top: 15px; display: none;">
                ...
                <div class="form-group" id="linkFormGroupAlamat" style="margin-top: 15px; display: none;">
                    <label for="alamat_pelatihan">Alamat <span style="color: red">(*)</span></label>
                    <textarea name="alamat_pelatihan" id="alamat_pelatihan" class="form-control" cols="10" rows="5"></textarea>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Select elements
            const pelatihanSelect = $('#pelatihan_id');
            const typeSelect = $('#type_pelatihan');
            const linkFormGroup = $('#linkFormGroup');
            const linkFormGroupAlamat = $('#linkFormGroupAlamat');

            // Form fields to show/hide
            const judulPelatihanGroup = $('#judul_pelatihan').closest('.form-group');
            const fotoGroup = $('#foto').closest('.form-group');
            const videoGroup = $('#video').closest('.form-group');
            const tipeGroup = typeSelect.closest('.form-group');
            const deskripsiGroup = $('#deskripsi').closest('.form-group');

            // Initialize all form fields as hidden
            judulPelatihanGroup.hide();
            fotoGroup.hide();
            videoGroup.hide();
            tipeGroup.hide();
            linkFormGroup.hide();
            ...
            // Function to toggle fields based on `pelatihan_id` selection
            pelatihanSelect.on('change', function() {
                ...
                if (selectedPelatihan === 'Pelatihan') {
                    // Show the relevant fields for 'Pelatihan'
                    judulPelatihanGroup.show();
                    fotoGroup.show();
                    videoGroup.show();
                    tipeGroup.show(); // Tipe selection dropdown
                    deskripsiGroup.show();

                    // Type select logic for 'Pelatihan'
                ...
                } else if (selectedPelatihan === 'Pengembangan') {
                    // Show fields for 'Pengembangan Diri'
                    judulPelatihanGroup.show();
                    fotoGroup.show();
                    videoGroup.show();
                    deskripsiGroup.show();
            ...
            $('#form_create_pelatihan').on('submit', function(e) {
                ...
                if ($('#pelatihan_id').val() === '') {
                    $('#pelatihan_id-error').text('Kategori Pelatihan wajib di isi.');
                    isValid = false;
                }

                if ($('#judul_pelatihan').val() === '') {
                    $('#judul_pelatihan-error').text('Judul Pelatihan wajib di isi.');
                    isValid = false;
                }

                if ($('#pelatihan_id').val() === 'Pelatihan') {
                    // Clear previous error messages
                    $('#type_pelatihan-error').text('');

                    // Check if 'type_pelatihan' has a value
                    if ($('#type_pelatihan').val() === '') {
                        $('#type_pelatihan-error').text('Tipe Latihan wajib di isi.');
                        isValid = false;
                    }
                }

                if ($('#deskripsi').val() === '') {
                    $('#deskripsi-error').text('Deskripsi wajib di isi.');
                    isValid = false;
                }

                const imageFile = $('#foto')[0].files[0];
                if (imageFile) {
                    ...
                    $('#foto-error').text('Foto wajib di isi.');
                    isValid = false;
                }
                ...
                // AJAX Request
                $.ajax({
                    url: '{{ url('/api/vendor/pelatihan_saya/create') }}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
