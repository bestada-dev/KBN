            ...
            <form id="form_edit_pengembangan" class="p-4 pt-3" enctype="multipart/form-data">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item"><span>Update Property</span></li>
                    </ol>
                </nav>
                ...
                <div class="form-group" id="show_if_type_link" style="margin-top: 15px; display: none;">
                    ...
                    <input type="text" name="url" value="{{ old('url', $data_edit->url) }}" id="url" class="form-control" placeholder="Enter url">
                            ...
                            <table class="table table-striped table-hover table-bordered">
                                ...
                                <tbody>
                                    @foreach ($data_edit['getAttachment'] as $index => $detailFoto)
                                        <tr id="detail-photo-row-{{ $index }}">
                                            <td class="text-center">
                                                <!-- File Name that opens modal for preview -->
                                                <a href="#" class="file-name" data-bs-toggle="modal" data-bs-target="#imagePreviewModal_{{ $index }}" onclick="showImagePreview('{{ asset('/detail_photos/'.$detailFoto['detail_photo']) }}', {{ $index }})">
                                                    {{ $detailFoto['detail_photo'] }}
                                                </a>
                                            </td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-danger btn-sm " id="btn-delete" data-id="{{ $detailFoto->id }}">
                                                    <i class="bi bi-trash"></i> Delete
                                                </button>
                                            </td>
                                        </tr>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body text-center">
                                                        ...
                                                        <img id="modalImagePreview_{{ $index }}" src="" alt="Preview Gambar" class="modal-image-preview" style="max-width: 100%; max-height: 400px; display: none;">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
                                                    </div>
...
@section('js')
    ...
    <script>
        function onCancel() {
            ...
            }, 'Are you sure?', 'You want the cancel to Update Property', 'Yes', 'No')
    ...
    <script>
        function showImagePreview(imageUrl, index) {
            ...
            modalImagePreview.style.display = 'block';  // Show the image inside the modal
        }

        $(document).on('click', '#btn-delete', function () {
            // Ambil semua checkbox yang dicentang
            var id = $(this).attr('data-id')
            var valueProperty_id = $('#property_id').val()
            // Tampilkan konfirmasi sebelum menghapus
            __swalConfirmation(async (data) => {
                try {
                    // Kirim permintaan DELETE untuk setiap ID
                    const res = await fetch(`{{ url('api/superadmin/property/delete-image') }}`, customPost({
                    ...
                    if (status) {
                        ...
                        setTimeout(function() {
                            // window.location.href = `{{ url('/superadmin/property/update') }}/${valueProperty_id}`;
                            location.reload();
