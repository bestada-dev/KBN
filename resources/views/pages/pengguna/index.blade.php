...
@section('breadcumbTitle', 'Pengguna')

@section('breadcumbSubtitle', 'Pengguna List')
    ...
    <article class="">
        ...
        <div class="SEARCH">
            <div class="row w-100">
                ...
                <div class="col-md-4">
                    <form>
                        <select class="form-control form-control-sm" id="search-statuses" onchange="searhBystatus(event)">
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header between">
                    <strong class="" id="detailMediaModalLabel" style="font-size: 1.2rem;color: #2778c4">Tambah Pengguna</strong>
                    <button type="button" class="btn btn-sm p-0" data-bs-dismiss="modal" aria-label="Close"
                        id="btn-close-request-media-modal">
                        <img src="{{ asset('assets/close.png') }}" width="70%" />
                    </button>
                </div>
                <form id="request-pengguna-form" method="POST" enctype="multipart/form-data">
...
@section('js')
    <script>
        ...
        $(document).on('click', '#btn-delete', function () {
            ...
            __swalConfirmation(async (data) => {
                try {
                    ...
                    const res = await fetch(`{{ url('api/superadmin/pengguna/delete') }}`, customPost({
                    ...
                    if (status) {
                        ...
                        setTimeout(function() {
                            window.location.href = `{{ url('/superadmin/pengguna') }}`
        ...
        $(document).ready(function() {
            ...
            // Saat form disubmit
            $('#request-pengguna-form').on('submit', function(e) {
                e.preventDefault();
                ...
                __isBtnSaveOnProcessing(__querySelector('#request-pengguna-form #btn-save'), true);

                if ($('input[name="user_id"]').val() === '') {
                    url = `{{ url('api/superadmin/pengguna/create') }}`;
                } else {
                    url = `{{ url('api/superadmin/pengguna/update') }}/${$('input[name="user_id"]').val()}`;
                    ...
                    success: function(response) {
                        if (response.success) {
                            __isBtnSaveOnProcessing(__querySelector(
                                '#request-pengguna-form #btn-save'), false);
        ...
        function DataTables(searchTerm, categoryId = undefined) {
            ...
            var ajaxConfig = {
                ...propertyDB,
                ajax: {
                    url: `{{ url('api/superadmin/pengguna/data-table') }}`,
                    dataType: 'JSON',
                    type: 'POST',
