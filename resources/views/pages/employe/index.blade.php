...
@section('breadcumbTitle', 'Employe')

@section('breadcumbSubtitle', 'Employe List')
                ...
                <div class="col-md-12">
                    <div class="d-flex gap-3 itemscenter">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control form-control-sm" placeholder="Cari Karyawan..."
                                id="search-datatable">
                        </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header between">
                    <strong class="" id="detailMediaModalLabel" style="font-size: 1.2rem;color: #2778c4">Sunting
                        Employe</strong>
                    <button type="button" class="btn btn-sm p-0" data-bs-dismiss="modal" aria-label="Close"
                ...
                <form id="request-employe-form" method="POST" enctype="multipart/form-data">
...
@section('js')
    <script>
        ...
        $(document).on('click', '#btn-delete', function () {
            ...
            __swalConfirmation(async (data) => {
                try {
                    ...
                    const res = await fetch(`{{ url('api/company/employe/delete') }}`, customPost({
                    ...
                    if (status) {
                        ...
                        setTimeout(function() {
                            window.location.href = `{{ url('/company/employe') }}`
        ...
        $(document).ready(function() {
            ...
            // Saat form disubmit
            $('#request-employe-form').on('submit', function(e) {
                e.preventDefault();
                ...
                __isBtnSaveOnProcessing(__querySelector('#request-employe-form #btn-save'), true);

                if ($('input[name="user_id"]').val() === '') {
                    url = `{{ url('api/company/employe/create') }}`;
                } else {
                    url = `{{ url('api/company/employe/update') }}/${$('input[name="user_id"]').val()}`;
                    ...
                    success: function(response) {
                        if (response.success) {
                            __isBtnSaveOnProcessing(__querySelector(
                                '#request-employe-form #btn-save'), false);
        ...
        function DataTables(searchTerm, categoryId = undefined) {
            ...
            var ajaxConfig = {
                ...propertyDB,
                ajax: {
                    url: `{{ url('api/company/employe/data-table') }}`,
                    dataType: 'JSON',
                    type: 'POST',
